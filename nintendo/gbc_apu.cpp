...
void gbc::apu_tick()
{
	...
	if( chan[0].active )
	{
		...
		total += duty[(io[0x11]>>6)*8 + chan[0].phase]>>4;
	}
	...
	if( chan[1].active )
	{
		...
		total += duty[(io[0x16]>>6)*8 + chan[1].phase]>>4;
	}
...
void gbc::div_apu_tick()
{
	...
	{
		chan[0].length += 1;
		if( chan[0].length == 64 ) chan[0].active = false;
	}
	if( chan[1].active && (io[0x19]&BIT(6)) )
	{
		chan[1].length += 1;
		if( chan[1].length == 64 ) chan[1].active = false;
	}
