...
// differences from SH4:
// - no fpu
// - interrupt handling uses R15 as a stack pointer, eg RTE and TRAPA

struct sh2_instr { u16 mask,res; std::function<void(sh2&,u16)> func; std::string name, decode; };
#define INSTR [](sh2& cpu, u16 opc) 
...
{ 0xFF00, 0xC300, INSTR {	
		cpu.r[15] -= 4;
		cpu.write(cpu.r[15], cpu.sr.v, 4);
		cpu.r[15] -= 4;
		cpu.write(cpu.r[15], cpu.pc+2, 4);
		cpu.pc = cpu.read(cpu.VBR + ((opc&0xff)<<2), 32);
	}, "trapa #imm", "11000011iiiiiiii" },
};

void sh2::exec(u16 opc)
{
...
void sh2::setSR(u32 v)
{
	sr.v = v;
	sr.b.pad22 = sr.b.pad2 = 0;
}

void sh2::div1(u32 m, u32 n)
...
void sh2::init()
{
	...
	{
		opcache[Op] = [&](sh2&, u16 opc) { std::println("${:X}: Unimpl opc ${:X}", pc, opc); exit(1); };
		for(const auto& t : sh2_opcodes)
		{
			if( (t.mask & Op) == t.res )
			{
				opcache[Op] = t.func;
