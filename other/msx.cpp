...
u8 msx::in(u16 p)
{
	...
	if( p == 0xA9 )
	{
		...
		if( kbrow == 0 )
		{
			if( keys[SDL_SCANCODE_7] ) v ^= BIT(7);
			if( keys[SDL_SCANCODE_6] ) v ^= BIT(6);
			if( keys[SDL_SCANCODE_5] ) v ^= BIT(5);
			if( keys[SDL_SCANCODE_4] ) v ^= BIT(4);
			if( keys[SDL_SCANCODE_3] ) v ^= BIT(3);
			if( keys[SDL_SCANCODE_2] ) v ^= BIT(2);
			if( keys[SDL_SCANCODE_1] ) v ^= BIT(1);
			if( keys[SDL_SCANCODE_0] ) v ^= BIT(0);
			return v;
		}
		if( kbrow == 1 )
		{
			if( keys[SDL_SCANCODE_SEMICOLON] ) v ^= BIT(7);
			if( keys[SDL_SCANCODE_RIGHTBRACKET] ) v ^= BIT(6);
			if( keys[SDL_SCANCODE_LEFTBRACKET] ) v ^= BIT(5);
			if( keys[SDL_SCANCODE_BACKSLASH] ) v ^= BIT(4);
			if( keys[SDL_SCANCODE_EQUALS] ) v ^= BIT(3);
			if( keys[SDL_SCANCODE_MINUS] ) v ^= BIT(2);
			if( keys[SDL_SCANCODE_9] ) v ^= BIT(1);
			if( keys[SDL_SCANCODE_8] ) v ^= BIT(0);
			return v;
		}
		if( kbrow == 2 )
		{
			if( keys[SDL_SCANCODE_B] ) v ^= BIT(7);
			if( keys[SDL_SCANCODE_A] ) v ^= BIT(6);
			if( keys[SDL_SCANCODE_SLASH] ) v ^= BIT(4);
			if( keys[SDL_SCANCODE_PERIOD] ) v ^= BIT(3);
			if( keys[SDL_SCANCODE_COMMA] ) v ^= BIT(2);
			if( keys[SDL_SCANCODE_GRAVE] ) v ^= BIT(1);
			if( keys[SDL_SCANCODE_APOSTROPHE] ) v ^= BIT(0);
			return v;
		}
		if( kbrow == 3 )
		{
			if( keys[SDL_SCANCODE_J] ) v ^= BIT(7);
			if( keys[SDL_SCANCODE_I] ) v ^= BIT(6);
			if( keys[SDL_SCANCODE_H] ) v ^= BIT(5);
			if( keys[SDL_SCANCODE_G] ) v ^= BIT(4);
			if( keys[SDL_SCANCODE_F] ) v ^= BIT(3);
			if( keys[SDL_SCANCODE_E] ) v ^= BIT(2);
			if( keys[SDL_SCANCODE_D] ) v ^= BIT(1);
			if( keys[SDL_SCANCODE_C] ) v ^= BIT(0);
			return v;
		}
		if( kbrow == 4 )
		{
			if( keys[SDL_SCANCODE_R] ) v ^= BIT(7);
			if( keys[SDL_SCANCODE_Q] ) v ^= BIT(6);
			if( keys[SDL_SCANCODE_P] ) v ^= BIT(5);
			if( keys[SDL_SCANCODE_O] ) v ^= BIT(4);
			if( keys[SDL_SCANCODE_N] ) v ^= BIT(3);
			if( keys[SDL_SCANCODE_M] ) v ^= BIT(2);
			if( keys[SDL_SCANCODE_L] ) v ^= BIT(1);
			if( keys[SDL_SCANCODE_K] ) v ^= BIT(0);
			return v;
		}
		if( kbrow == 5 )
		{
			if( keys[SDL_SCANCODE_Z] ) v ^= BIT(7);
			if( keys[SDL_SCANCODE_Y] ) v ^= BIT(6);
			if( keys[SDL_SCANCODE_X] ) v ^= BIT(5);
			if( keys[SDL_SCANCODE_W] ) v ^= BIT(4);
			if( keys[SDL_SCANCODE_V] ) v ^= BIT(3);
			if( keys[SDL_SCANCODE_U] ) v ^= BIT(2);
			if( keys[SDL_SCANCODE_T] ) v ^= BIT(1);
			if( keys[SDL_SCANCODE_S] ) v ^= BIT(0);
			return v;
		}
		if( kbrow == 6 )
		{
			...
			if( keys[SDL_SCANCODE_LSHIFT] || keys[SDL_SCANCODE_RSHIFT] ) v ^= BIT(0);
			return v;
		}
		if( kbrow == 7 )
		{
			if( keys[SDL_SCANCODE_RETURN] ) v ^= BIT(7);
			//if( keys[SDL_SCANCODE_F2] ) v ^= BIT(6);
			if( keys[SDL_SCANCODE_BACKSPACE] ) v ^= BIT(5);
			//if( keys[SDL_SCANCODE_O] ) v ^= BIT(4);
			if( keys[SDL_SCANCODE_TAB] ) v ^= BIT(3);
		...
		if( kbrow == 8 )
		{
			...
			if( keys[SDL_SCANCODE_SPACE] ) v ^= BIT(0);
			return v;
		}
		return 0xff;
	}
